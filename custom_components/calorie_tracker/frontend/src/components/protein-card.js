class ProteinGaugeCard extends HTMLElement {
  constructor() {
    super();
  ...
  async _updateCard() {
    const wrapper = this.querySelector('.protein-gauge');
  ...
  this._applyProteinStyles(wrapper);
  ...
  this._renderGauge();
      ...
      console.error('Failed to fetch protein data:', err);
    }
  ...
  _applyProteinStyles(wrapper) {
    if (!wrapper) return;
    // Basic container sizing
    const svg = wrapper.querySelector('.protein-svg');
    if (svg) {
      svg.style.width = '100%';
...
if (!customElements.get('protein-gauge-card')) {
...
window.customCards.push({
  ...
  editor: 'protein-gauge-editor',
  preview: true,
});

class ProteinGaugeEditor extends HTMLElement {
  setConfig(config) {
    this._config = config || {};
    this._render();
  }
...
if (!customElements.get('protein-gauge-editor')) {
  customElements.define('protein-gauge-editor', ProteinGaugeEditor);
}
// Provide built-in visual editor schema for Lovelace
ProteinGaugeCard.getConfigForm = function () {
  return {
    schema: [
...
ProteinGaugeCard.getStubConfig = function () {
...
ProteinGaugeCard.prototype._renderGauge = function () {
  ...
  const value = Math.max(0, this._proteinValue || 0);
  const min = this._proteinMin;
  const max = this._proteinMax;
