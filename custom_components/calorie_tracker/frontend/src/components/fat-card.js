class FatGaugeCard extends HTMLElement {
  ...
  this._applyFatStyles(wrapper);
    ...
    try {
      ...
      let computedMax = Math.ceil((defaultWeightLb * 0.36) / 10) * 10;
  ...
  this._fatMin = resolvedMin;
  this._fatMax = resolvedMax;
...
window.customCards.push({
  ...
  editor: 'fat-gauge-editor',
  preview: true,
});

class FatGaugeEditor extends HTMLElement {
  setConfig(config) {
    this._config = config || {};
    this._render();
  }
...
if (!customElements.get('fat-gauge-editor')) {
  customElements.define('fat-gauge-editor', FatGaugeEditor);
}

// Provide built-in visual editor schema for Lovelace
FatGaugeCard.getConfigForm = function () {
  return {
    schema: [
...
FatGaugeCard.getStubConfig = function () {
